    ...
    if (ext === '.json') {
        ...
        if (_.isObject(json)) {
            ...
            Object.keys(json).forEach(function (key) {
                var judgment1 = path.basename(json[key]).replace(new RegExp( opts.revSuffix ), '' ) !==  path.basename(key);
                var judgment2 = path.basename(json[key]).split('?')[0] !== path.basename(key);
